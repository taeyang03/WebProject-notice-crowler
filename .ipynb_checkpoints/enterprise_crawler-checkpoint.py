...
async def crawl_and_update_db():
    ...
    for doc in all_new_docs:
        ...
        if doc_id not in seen_ids:
            ...
            logger.info(f"  ↳ 중복 수집 필터링됨 (DB 삽입 제외): {doc.metadata.get('title', '')}")

    if not unique_docs:
        logger.info("모든 공지가 중복 처리되어 DB에 추가할 내용이 없습니다.")
        return

    logger.info(f"총 {len(unique_docs)}개의 고유한 새 공지를 Chroma Vector DB에 추가합니다...")
    vector_store.add_documents(documents=unique_docs, ids=unique_ids)
    logger.info(f"{len(unique_docs)}건 DB 추가 완료.")
