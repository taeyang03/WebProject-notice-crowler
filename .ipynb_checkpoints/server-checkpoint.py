...
async def command_node(state: AgentState, config: RunnableConfig) -> dict:
    ...
    try:
        if cmd in ["!help", "!man"]:
            ...
            if logged_in_email:
                ...
                if user_data["ids"]:
                    ...
                    response_text = "DB에서 유저 정보를 찾을 수 없습니다. 다시 `!su` 해주세요."
            else:
            ...
            if not logged_in_email:
                ...
                if subcmd == "ls":
                    response_text = f"내 키워드: {', '.join(current_kws) if current_kws else '없음'}"
                elif subcmd == "add":
                    if len(parts) < 3:
        ...
        response_text = f"명령어 실행 중 오류 발생: {e}"
...
async def rag_node(state: AgentState, config: RunnableConfig) -> dict:
    ...
    # 프롬프트 내부의 개수 제한 지침을 유저 요청치({limit})로 동적 변경
...
async def summary_node(state: AgentState, config: RunnableConfig) -> dict:
    ...
    # 프롬프트 내부의 요약 선별 개수 제약을 유저 요청치({limit})로 동적 매핑
